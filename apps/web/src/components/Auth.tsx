...
export default function Auth() {
  ...
  const submit = async () => {
    setError(null)
    const r = await (mode === 'signup' ? signUpEmail(name, email, password) : signInEmail(email, password))
    ...
    <div style={{ minHeight: '100vh', display: 'flex', background: 'var(--bg)' }}>
      ...
      <div style={{ width: 480, maxWidth: '100%', flex: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 28px' }} className="auth-form">
        <div style={{ width: 360, maxWidth: '100%' }}>
          ...
          {/* Bannière explicative pour l'application web */}
          <div style={{
            display: 'flex',
            ...
            <div>
              <strong>Mode web connecté</strong> : L'application web enregistre vos données (conversations, clés, agents) dans la base PostgreSQL. Les connexions créent un compte lié en base.
            </div>
          </div>
