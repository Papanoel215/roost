...
export function upsertConversation(conv: StoredConversation): void {
  ...
  if (email !== 'default') {
    ...
    }).catch(err => console.error('Failed to sync conversation:', err))
  }
}
  ...
  try {
    ...
    if (profileRes.ok) {
      ...
      if (profileData.settings && Object.keys(profileData.settings).length > 0) {
        state = {
          profile: { ...DEFAULT.profile, ...(profileData.settings.profile || {}) },
          keys: { ...DEFAULT.keys, ...(profileData.settings.keys || {}) },
          tested: { ...DEFAULT.tested, ...(profileData.settings.tested || {}) },
          notif: { ...DEFAULT.notif, ...(profileData.settings.notif || {}) },
          mfa: typeof profileData.settings.mfa === 'boolean' ? profileData.settings.mfa : DEFAULT.mfa,
        }
        localStorage.setItem(`roost.settings.${email}`, JSON.stringify(state))
        subs.forEach((f) => f())
      }
    }
    ...
    if (convosRes.ok) {
      ...
      if (convosData.conversations) {
        convosState = convosData.conversations
        localStorage.setItem(`roost.conversations.${email}`, JSON.stringify(convosState))
        convosSubs.forEach((f) => f())
      }
    }
  } catch (err) {
