...
export function signInProvider(provider: 'google' | 'facebook' | 'apple') {
  const domain = provider === 'google' ? 'gmail.com' : provider === 'apple' ? 'icloud.com' : 'facebook.com'
  const email = `maxime@${domain}`
  if (!accounts[email]) { accounts[email] = { email, name: 'Maxime', provider }; saveAccounts() }
  start(accounts[email])
}

export function signOut() { session = null; saveSession(); emit() }
